Surfaces.prototype.sphere = (count = 10, r = 1, teta, fi) => {

    const points = [];
    for (let teta = 0; teta <= Math.PI; teta += Math.PI / count) {
        for (let fi = 0; fi < Math.PI * 2; fi += Math.PI / count) {
            points.push(new Point(
                r * Math.cos(fi) * Math.sin(teta),
                r * Math.sin(fi) * Math.sin(teta),
                r * Math.cos(teta)
            ));
        }
    }
    ...
    /* edges.push(new Edge(points.length - 1, 0)); //-соединяет в сфере начало и конец*/
